...
from .data import Position, AsterEphemerides, MoonPhase, ASTERS, skyfield_to_moon_phase
...
def get_ephemerides(date: datetime.date, position: Position) -> [AsterEphemerides]:
    ...
    for aster in ASTERS:
        def get_angle(time: Time) -> float:
            return position.get_planet_topos().at(time).observe(aster.get_skyfield_object()).apparent().altaz()[0]\
                   .degrees
        ...
        ephemerides.append(AsterEphemerides(rise_time, culmination_time, set_time, object=aster))
